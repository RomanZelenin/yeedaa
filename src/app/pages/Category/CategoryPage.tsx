...
export default function CategoryPage() {
    ...
    useEffect(() => {
        fetchRecepies().then((recipes) => {
            let recepiesFromCurrentCategory = recipes.filter((recepie) =>
                recepie.category.includes(category!),
            );

            recepiesFromCurrentCategory = filterRecipesByAllergens(
                recepiesFromCurrentCategory,
                allergens,
            );
            if (query.length > 0) {
                recepiesFromCurrentCategory = filterRecipesByTitle(
                    recepiesFromCurrentCategory,
                    query,
                );
            }
                    ...
                    <ContentContainer title={titleCategory} subtitle={subtitleCategory}>
                        <>
                            <Tabs
                                ...
                                index={tabIndex}
                                onChange={(idx) => {
                                    const s = menuItems.find(
                                        (it) => it.path!.substring(1) === category,
                                    )!.subcategory![idx];
                            ...
                            >
                                ...
                                >
                                    {menuItems
                                        .find((it) => it.path?.substring(1) === category)
                                        ?.subcategory?.map((it) => (
                                            <Tab color='lime.800'>{it.title}</Tab>
                                        ))}
                                </TabList>
